...
export class EditDrugComponent implements OnInit {
  ...
  public setReviseOrderValues(reviseOrders) {
    ...
    if (reviseOrders.dateStopped) {
      ...
      this.encounterResourceService
        .saveEncounter(encounterPayLoad)
    ...
    this.dosageFormv = reviseOrders.quantityUnits.uuid;
  ...
  public setOrderValues(selectedDrug) {
    if (!this.draftedDrug) {
      ...
      if (selectedDrug.orderTemplate) {
        ...
        this.dosageFormv = this.options.quantityUnits[0].uuid;
        this.quantity = 1;
        this.refills = 2;
      }
    } else {
  ...
  public createSentence() {
    ...
    this.drugOrderService.PharmacyDrugInfo(this.sentence).subscribe((res) => {
        this.PharmacyDrug = res; 
        this.PharmacyDrug.DefaultSellingPrice = this.PharmacyDrug.DefaultSellingPrice.toFixed(2);
        if(this.PharmacyDrug.InStock > 1){
  ...
  public calculateQuantity(value) {
    if (!this.customDrug) {
      ...
      this.SellingPrice = (this.quantity * this.PharmacyDrug.DefaultSellingPrice).toFixed(2);
  ...
  public getOrderEntryConfigs() {
    this.orderResourceService.getOrderEntryConfig().subscribe((data) => {
      ...
      this.doseUnits = data.drugDosingUnits;
    });
  }
  ...
  public saveOrderFinal(drugOrderPayload) {
    this.orderResourceService.saveDrugOrder(drugOrderPayload).subscribe(
      (res) => {
        ...
        if (this.submittedDrugOrder.orderNumber) {
          setTimeout(() => {
            ...
            this.successMessage =
              ...
              name +
              "" +
              "Successful as Order No:" +
              this.submittedDrugOrder.orderNumber;
          }, 1000);
          setTimeout(() => {
            this.editDrug = false;
            this.successMessage = "";
      ...
      (error) => {
        this.errors.push({
          id: "similar drug",
          message:
            "Patient is under similar medication. Discontinue the other order",
        });
      }
    );
  }
  public createEncounterPayload() {
    return {
      patient: this.patient.uuid,
      encounterType: "40cd3cff-23b6-41da-93b9-549fa5abdfc0",
