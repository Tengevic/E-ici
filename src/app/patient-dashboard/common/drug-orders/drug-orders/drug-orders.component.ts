...
export class DrugOrdersComponent implements OnInit {
  ...
  public PharmacyDraft = [];
  public PatientDetail;
  public eiciNo;
  public sentTophamarcy = false;
  public errorTophamarcy = false;
  public errorPhamarcy;
  ...
  public getProvider() {
    this.drugOrderService
      .getProviderByPersonUuid(this.personUuid)
      .subscribe((data) => {
  ...
  public getDrugOrders() {
    ...
    this.orderResourceService
      .getAllOrdersByPatientUuuid(patientUuId, this.caresetting)
      .subscribe((data) => {
        data = data.results;
        data.forEach((value) => {
  ...
  public processStopValue() {
    this.display = false;
    this.orderResourceService
      .getOrderByUuid(this.orderValue.uuid)
      .subscribe((data) => {
        if (!data.dateStopped) {
          const discontinuePayload = this.createPayload(data, "DISCONTINUE");
          if (
            window.confirm("Are You Sure You Want To Discontnue This Drug?")
          ) {
            this.drugOrderService
              .saveOrder(discontinuePayload)
              .subscribe((success) => {
  ...
  public processRenewOrder(orderValue) {
    ...
    this.orderResourceService.getOrderByUuid(orderValue).subscribe((data) => {
      if (data.dateStopped) {
        ...
        this.drugOrderService.saveOrder(renewPayload).subscribe(
          (success) => {
        ...
        const discontinuePayload = this.createPayload(data, "DISCONTINUE");
        const dis = this.drugOrderService
          .saveOrder(discontinuePayload)
          .subscribe(
            (success) => {
              return true;
  ...
  public viewOrder(order) {
    const payload = {
      ...
      regimen: "nleomyciun",
      goal: _.isObject(order.orderReason)
        ? order.orderReason.display.display
        : order.orderReason,
      // goal: 'test',
    ...
    this.encounterService
      ...
      .subscribe((encounter) => {
        encounter.forEach((encounterSelected) => {
          if (
            this.dateTransformation(encounterSelected.encounterDatetime) ===
            this.dateTransformation(order.dateActivated)
          ) {
            if (
              encounterSelected.encounterType.uuid ===
                "2cd62224-5507-48ee-a7b3-55f66162b148" ||
              "5fa823ce-7592-482f-a0aa-361abf326ade"
            ) {
              this.encounterService
                .getEncounterByUuid(encounterSelected.uuid)
                .subscribe((encounterDetails) => {
                  if (
                    encounterSelected.encounterType.uuid ===
                    "2cd62224-5507-48ee-a7b3-55f66162b148"
                  ) {
                    ...
                    encounterDetails.obs.forEach((ob) => {
                      ...
                      if (
                        obsNeeded === "a8a6619c-1350-11df-a1f1-0026b9348838"
                      ) {
                        payload.height = ob.value;
                      } else if (
                        obsNeeded === "a898c87a-1350-11df-a1f1-0026b9348838"
                      ) {
                        payload.bsa = ob.value;
                      } else if (
                        obsNeeded === "a8a660ca-1350-11df-a1f1-0026b9348838"
                      ) {
  ...
  public SendToPharmacy(order){
    console.log(order);
     let phamPatient=  new Patient({});
     phamPatient = this.patient;
     console.log(phamPatient._identifier[0]);
    if(!this.PatientDetail){ 
    this.PatientDetail = {
      uuid : this.patient.uuid,
      Name : this.patient.person.display,
      type : 1,
      city : this.patient.person.preferredAddress.display
    }
    let identifer= order.patient.identifiers[0];
    var eici = identifer.display;
    var split = eici.split("=", 2);
    this.eiciNo = split[1];
    console.log(this.eiciNo);
    };
    var orderNumber = order.orderNumber;
    var orderSplit = orderNumber.split("-",2);
    var orderId = orderSplit[1];
    this.PharmacyDraft.push({
      OrderId: orderId,
      prescription: order.display
    })
   
  }
  public ClearPharmacy(){
    this.PharmacyDraft = [];
    this.PatientDetail = null;
    this.eiciNo = null;
  }
  public ToPharmacy(){
    this.drugOrderService.PostToPharmacy({
      eiciNo :this.eiciNo,
      PatientDetail :this.PatientDetail,
      prescription: this.PharmacyDraft
    }).subscribe(res => {
       console.log(res)
       this.ClearPharmacy();
       this.sentTophamarcy = true;
       setTimeout(()=>{                          
        this.sentTophamarcy = false;
      }, 5000);
      }, (error)=>{
          this.errorTophamarcy = true;
          this.errorPhamarcy = error;
          setTimeout(()=>{                          
            this.errorTophamarcy = false;
          }, 5000);
      });
  }
