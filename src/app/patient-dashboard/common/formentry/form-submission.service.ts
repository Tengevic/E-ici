...
export class FormSubmissionService {
  ...
  private submitEncounterPayload(
    ...
    if (encounterPayload.uuid) {
      ...
      const encounterPayloadNew = this.submitEncounterWithOrdersFromotheEncounter(
        form,
        encounterPayload
      );

      return this.encounterResourceService.saveEncounter(encounterPayloadNew);
    }
  }

  public submitEncounterWithOrdersFromotheEncounter(form, encounterPayload) {
    let obs = [];
    console.log("form", form);
    if (!_.isUndefined(form.dataSourcesContainer.dataSources.order)) {
      const labPayload = JSON.parse(
        form.dataSourcesContainer.dataSources.order.labOrder
      );
      encounterPayload.obs.forEach((ob) => {
        if (labPayload.orderConcept == ob.concept) {
          obs.push({
            concept: ob.concept,
            order: labPayload.orderuuid,
            value: ob.value,
          });
        } else {
          obs.push({
