...
export class FormentryComponent implements OnInit, OnDestroy {
    ...
    this.route.params.subscribe((routeParams) => {
      if (routeParams) {
        ...
        this.route.queryParams.subscribe((params) => {
          ...
          componentRef.order = params["order"];
  ...
  public onSubmit(): void {
    console.log("test");
    // t;his.isBusyIndicator(true, 'Please wait, Submitting form');
    setTimeout(() => {
  ...
  public navigateTo(path): void {
    let visitUrl = this.localStorageService.getItem("visitUrl");
    switch (path) {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (step === "landing-page") {
        ...
        this.router.navigate(
          ...
          { queryParams: { encounter: this.encounterUuid } }
        );
      } else if (step === "edit") {
        this.router.navigate([
          "/patient-dashboard/patient/" +
    ...
    this.formSubmissionService
      ...
      .subscribe(
        (data) => {
          ...
          this.encounterUuid = data[0].uuid;
