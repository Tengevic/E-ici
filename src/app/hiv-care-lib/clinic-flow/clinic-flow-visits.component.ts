...
export class ClinicFlowVisitsComponent implements OnInit, OnDestroy {
      ...
      .subscribe((clinic) => {
        ...
        this.selectedDateSubscription = this.clinicFlowCacheService
          ...
          .subscribe((date) => {
            ...
            this.maxSelectedDateSubscription = this.clinicFlowCacheService
              .getmaxSelectedDate()
              .subscribe((maxDate) => {
  ...
  public ngOnDestroy(): void {
    ...
    if (this.maxSelectedDateSubscription) {
      this.maxSelectedDateSubscription.unsubscribe();
    }
  ...
  public getClinicFlow(dateStated, maxSelectedDate, locations) {
    this.initParams();
    this.loadingClinicFlow = true;
    this.clinicFlowCacheService.setIsLoading(this.loadingClinicFlow);
    const result = this.clinicFlowResource.getClinicFlow(
      dateStated,
      maxSelectedDate,
      locations
    );
