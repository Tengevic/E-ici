...
export class ProgramManagerBaseComponent implements OnInit {
  ...
  public mapEnrolledProgramsToServiceOffered(
    ...
    if (this.enrolledProgrames) {
      ...
      _.each(this.serviceOfferedConf, (config: any) => {
        let medicalservicePrograms = _.intersectionWith(
          enrolledProgrames,
          config.programs,
        ...
        if (excludeCompleted) {
          medicalservicePrograms = _.filter(
            medicalservicePrograms,
            (program) => {
  ...
  public getServiceOfferedConf() {
    this._servicesOfferedService
      .getserviceOfferedProgramsConfig()
      .pipe(take(1))
      .subscribe((results) => {
        if (results) {
  ...
  public getProgramsByMedicalServiceName(): any[] {
    ...
    if (medicalService) {
      return _.filter(medicalService.programs, (program) => {
        ...
        if (program.allowedLocations.length > 0) {
          if (
            _.includes(programs, program.uuid) &&
            _.includes(program.allowedLocations, this.selectedLocation.value)
          ) {
            return true;
          } else {
            return false;
  ...
  private _filterServicesOfferedConfigByName() {
    this.programMedicalServices = _.map(
      this.serviceOfferedConf,
      (config: any) => {
